...
describe("INVERSION_TYPES", () => {
  it("contains exactly 4 types", () => {
    expect(INVERSION_TYPES).toHaveLength(4)
  })

  it("includes all expected inversion qualities", () => {
    expect(INVERSION_TYPES).toContain("major")
    expect(INVERSION_TYPES).toContain("minor")
    expect(INVERSION_TYPES).toContain("diminished")
    expect(INVERSION_TYPES).toContain("augmented")
  })
})

describe("INVERSION_STRING_SETS", () => {
  it("lists 10 string sets in canonical order (close first, then open)", () => {
    expect(INVERSION_STRING_SETS).toHaveLength(10)
    // Close sets first: root 6→3
    expect(INVERSION_STRING_SETS[0]).toBe("6-5-4")
    expect(INVERSION_STRING_SETS[1]).toBe("5-4-3")
    expect(INVERSION_STRING_SETS[2]).toBe("4-3-2")
    expect(INVERSION_STRING_SETS[3]).toBe("3-2-1")
    // Open sets after: root 6→4
    expect(INVERSION_STRING_SETS[4]).toBe("6-5-3")
    expect(INVERSION_STRING_SETS[9]).toBe("4-2-1")
  })
})
...
describe("getInversionVoicings", () => {
  ...
  it("returns voicings for every tonic × type combination", () => {
    ...
    for (const tonic of tonics) {
      for (const type of INVERSION_TYPES) {
